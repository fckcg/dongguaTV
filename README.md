...
- **原理**：对正在播放的音频做轻量频谱指纹（WebAudio，每 0.5s 一个 16 维向量，**零额外带宽**）→ 两集滑动互相关找出 ≥20s 的公共段 = 片头（两集冷开场长短不同也能对上）→ 后续集实时匹配检测片头起点。**画面不参与**（不同码率/水印会失效，音频对编码差异不敏感）。
...
- **限制**：iOS Safari 原生 HLS 下取不到音频采样 → iOS 用户只**消费**共享标记（桌面/安卓用户学一遍全站受益）；综艺无固定片头曲，天然学不出（属正常）；倍速观看时不学习。

...
- **原理**：音频频谱指纹（每 0.5s 一个 16 维向量）→ 两集滑动互相关找出 ≥20s 的公共段 = 片头（冷开场长短不同也能对上）。**画面不参与**（不同码率/水印会失效，音频对编码差异不敏感）。两条学习路径：
  - **离线抓 m3u8（首选，不用看片头）**：全站还没有共享标记时，后台抓这集+邻集的 m3u8（自动选**最低码率**省流量，每部剧首次约十来 MB）、拆 TS 取 AAC、`decodeAudioData` 解码、算指纹、互相关 → **秒出片头**，用户完全不用看。**iOS 也能用**（不依赖 WebAudio 采集）。个别 AES 加密源自动解密；无法解码的源回退下面这条。
  - **边看边学（兜底，零额外带宽）**：`createMediaElementSource` 挂在正在播放的音频上取指纹，看两集后学出、后续集实时匹配。iOS 原生 HLS 下取不到采样 → iOS 走离线路径或消费共享标记。
- **共享标记**：按 `(剧名, 线路, 集号)` 存服务器（`/api/intro/mark(s)`），多用户 ±5s 加权计票收敛（SponsorBlock 模式）；错值/被刷值由诚实多数逐票削正。**自动跳过只信被佐证（≥2 票或本机实测）的标记**，单条误学最多弹一个可忽略的按钮、绝不误跳正片。
